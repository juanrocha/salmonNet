...
## problems not finding Java:
dyn.load('/Library/Java/JavaVirtualMachines/jdk-12.0.2.jdk/Contents/Home/lib/server/libjvm.dylib')


# Initialize Chrome and go to WoS from SU website, so it's authenticated already.

## Initialize remote driver
d <- rsDriver(port = 4567L, browser = "chrome") # should open a chrome
remDr <- d[["client"]]
